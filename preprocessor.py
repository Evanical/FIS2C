...
class Preprocessor:
    ...
    def pad_wav(self, waveform, target_length):
        ...
        # Resampling can introduce a 1-sample (or small) mismatch.
        # If waveform is longer than target_length, crop safely.
        if waveform_length > target_length:
            return waveform[:, :target_length]

        # If waveform is shorter, zero-pad it.
        temp_wav = np.zeros(
            (waveform.shape[0], target_length),
            dtype=np.float32,
        )

        if self.pad_wav_start_sample is None:
            max_start = target_length - waveform_length
            rand_start = (
                int(self.random_uniform(0, max_start))
                if max_start > 0
                else 0
            )
        else:
            rand_start = int(self.pad_wav_start_sample)

        # Defensive clamp so the destination slice always fits.
        max_start = max(0, target_length - waveform_length)
        rand_start = max(0, min(rand_start, max_start))

        end = rand_start + waveform_length
        temp_wav[:, rand_start:end] = waveform
