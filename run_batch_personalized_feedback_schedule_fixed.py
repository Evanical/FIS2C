...
def ensure_personalized_prompt_ref(source_prompt, concept):
    """
    Ensure prompt_ref contains a bracketed concept required by
    SteerMusic_personalized_ablation_feedback_schedule.py.

    Example:
        source_prompt = "a guitar solo"
        concept       = "guitar"
        -> "a [guitar] solo"

    If source_prompt already contains [...], keep it unchanged.
    If concept cannot be found in source_prompt, return "" so the caller
    can skip the row instead of crashing the whole batch.
    """
    ...
    if re.search(r"\[[^\[\]]+\]", source_prompt):
        return source_prompt

    if not concept:
        return ""

    # Replace the first case-insensitive literal occurrence of concept.
    pattern = re.compile(re.escape(concept), flags=re.IGNORECASE)
    if pattern.search(source_prompt):
        return pattern.sub(f"[{concept}]", source_prompt, count=1)
...
def main():
    ...
    with status_path.open("a", encoding="utf-8") as f_status:
        for local_i, (_, row_obj) in enumerate(tqdm(df.iterrows(), total=len(df))):
            ...
            # Personalized inference requires prompt_ref to contain [concept].
            # Convert e.g. "a guitar solo" + concept="guitar"
            # into "a [guitar] solo". If the concept is absent, skip safely.
            raw_source_prompt = source_prompt
            source_prompt = ensure_personalized_prompt_ref(source_prompt, concept)

            audio_path = resolve_audio_path(row, args.audio_root, args.audio_key)

            if not raw_source_prompt:
            ...
            if not source_prompt:
                write_status(
                    f_status,
                    {
                        ...
                        "status": "cannot_build_personalized_prompt_ref",
                        "raw_source_prompt": raw_source_prompt,
                        "concept": concept,
                    },
                )
                print(
                    f"[SKIP] index={original_index}: concept={concept!r} "
                    f"not found in source_prompt={raw_source_prompt!r}"
                )
                continue
