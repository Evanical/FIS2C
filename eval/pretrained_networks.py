...
class CLAP_base(torch.nn.Module):
    def __init__(self, checkpoint_path='clap/pretrained', requires_grad=False, model_arch='HTSAT-base',
        ...
        # Compatibility fix:
        # Some transformers/Roberta versions register position_ids as a buffer,
        # while old CLAP checkpoints do not contain it.
        if "text_branch.embeddings.position_ids" in model_state and "text_branch.embeddings.position_ids" not in pkg:
            pkg["text_branch.embeddings.position_ids"] = model_state["text_branch.embeddings.position_ids"]

        missing, unexpected = self.model.model.load_state_dict(pkg, strict=False)
        if missing:
            print("[WARN] LPAPS/CLAP missing keys:", missing)
        if unexpected:
            print("[WARN] LPAPS/CLAP unexpected keys:", unexpected)
        self.model.eval()

        # compatibility patch
        if not hasattr(self.model, "enable_fusion"):
            self.model.enable_fusion = enable_fusion
    ...
    def forward(self, audio: torch.Tensor, sample_rates: torch.Tensor):
        ...
        temp_dict = {}
        # Compatibility fix for older/newer laion_clap versions:
        # some versions of get_audio_features do NOT support require_grad kwarg.
