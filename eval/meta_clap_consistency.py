...
def load_clap_state_dict(clap_model, path: tp.Union[str, Path]):
    ...
    model_state = clap_model.model.state_dict()
    ...
    missing, unexpected = clap_model.model.load_state_dict(pkg, strict=False)
    if missing:
        print("[WARN] CLAP missing keys:", missing)
    if unexpected:
        print("[WARN] CLAP unexpected keys:", unexpected)
