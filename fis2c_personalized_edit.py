...
def main():
    ...
    # Optional personalized .pt weights.
    # If no valid checkpoint is provided, personalization is driven by the
    # reference-audio branch in FISC instead of failing the run.
    personalized_ckpt_loaded = False
    if opt.personalized_ckpt:
        personalized_ckpt_path = Path(opt.personalized_ckpt).expanduser()
        if personalized_ckpt_path.is_file():
            personalized_ckpt_path = personalized_ckpt_path.resolve()
            print("[INFO] loading personalized weights:", personalized_ckpt_path)
            ckpt = torch.load(str(personalized_ckpt_path), map_location=DEVICE)
            ...
            print("[WARN] personalized checkpoint not found; using reference-audio personalization only:", personalized_ckpt_path)
    else:
        print("[INFO] no personalized checkpoint provided; using reference-audio personalization only")
    ...
    if use_fisc:
        ...
        if not opt.ref_audio_path:
            raise ValueError("--ref_audio_path is required for reference-audio personalization when FISC is enabled.")
        ref_path = Path(opt.ref_audio_path).expanduser()
        if not ref_path.is_file():
            raise FileNotFoundError(f"reference audio not found: {ref_path}")
        opt.ref_audio_path = str(ref_path.resolve())
        print("[INFO] resolved reference audio:", opt.ref_audio_path)
